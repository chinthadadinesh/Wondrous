...
exports.getuservotes = async (req, res) => {
    ...
    db.connection.query("select postId from votes where UserId=?",[id], function (err, rows) {
      if(rows.length !=0){
        res.send({
            code:200,
            rows:rows
     ...
     exports.select= async(req,res)=>{
        ...
        console.log(sql,wherevalue)
         ...
         exports.addvote = async (req, res) => {
            ...
            let userid = await sanitizer.escape(req.params.userid);
            db.connection.query('update posts set votes=votes+? where postId=?', [1,id], function (error, resl) {
                if (error) {
                    ...
                    db.connection.query('insert into votes (postId,userId) values(?,?)', [id,userid], function (err, resl) {
                        if(!err){
                            res.send({
                                "code": 200,
